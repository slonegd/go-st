...
type Context interface {
	...
	SetIdent(name string, val interface{})
}
...
func (ctx mapContext) SetIdent(name string, val interface{}) {
	ctx.mp[name] = val
}
...
func (ctx reflectContext) SetIdent(string, any) {}
...
func (ctx nilContext) SetIdent(string, any) {}
...
func (e *Expr) Eval(context interface{}) (val interface{}, err error) {
	...
	val, err = evalStmts(e.stmts, ctxt)
	if err != nil {
		return nil, err
	}
	if r, ok := val.(returnValue); ok {
		val = r.v
	}
	return val, nil
}
...
func (e *Expr) EvalNoRecover(context interface{}) (interface{}, error) {
	...
	//evaluate the prepared sentence
	return eval(e.stmts[0], ctxt) // не используем
}
...
func statements(expr string) ([]ast.Stmt, error) {
	fexpr := `package expr
func expr(){
` + expr + `
}`
	f, err := parser.ParseFile(token.NewFileSet(), "", []byte(fexpr), 0)
	if err != nil {
		return nil, err
	}
...
func evalStmts(stmts []ast.Stmt, ctxt Context) (val any, err error) {
	for _, stmt := range stmts {
		val, err = eval(stmt, ctxt) // TODO return
		if err != nil {
			return val, err
		}
...
func createContext(context interface{}) Context {
	...
	default:
		var ok bool
		if context == nil {
			ctxt = mapContext{mp: make(map[string]interface{})}
			// ctxt = nilContext{}
		} else {
...
func eval(expr ast.Node, context Context) (interface{}, error) {
	...
	case *ast.AssignStmt:
		err := evalAssignStmt(v, context)
		return nil, err
	case *ast.ExprStmt:
		return eval(v.X, context)
	// case *ast.BadDecl:
	// 	return nil, fmt.Errorf("%s not suported", reflect.TypeOf(expr))
	// case *ast.BadExpr:
	// 	return nil, fmt.Errorf("%s not suported", reflect.TypeOf(expr))
	...
	case *ast.BlockStmt:
		return evalBlockStmt(v, context)
		/*
			case *ast.BranchStmt:
				return nil, fmt.Errorf("%s not suported", reflect.TypeOf(expr))
		*/
...
func evalAssignStmt(expr *ast.AssignStmt, context Context) error {
	name := expr.Lhs[0].(*ast.Ident).Name
	v, err := eval(expr.Rhs[0], context)
	if err != nil {
		return err
	}
	context.SetIdent(name, v)
	return nil
}

func evalIfStmt(expr *ast.IfStmt, context Context) (any, error) {
	cond, err := eval(expr.Cond, context)
	if err != nil {
		return nilInterf, err
	}
	if isTrue(cond) {
		return evalBlockStmt(expr.Body, context)
	} else if expr.Else != nil {
		return eval(expr.Else, context)
	}
	return nilInterf, nil
}

func evalBlockStmt(expr *ast.BlockStmt, context Context) (any, error) {
	return evalStmts(expr.List, context)
}
...
func evalReturn(expr *ast.ReturnStmt, context Context) (any, error) {
	if len(expr.Results) == 0 {
		return returnValue{nil}, nil
	}
	v, err := eval(expr.Results[0], context)
	if err != nil {
		return nilInterf, err
	}
